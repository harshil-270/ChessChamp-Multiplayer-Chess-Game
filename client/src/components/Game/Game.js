...
function Game(props) {
    ...
    const [opponentDetails, setOpponentDetails] = useState({ username: '', rating: 0 });
    ...
    const checkWinStatus = async (lastMoveColor) => {
        ...
        if (winner !== '') {
            ...
            setShowPopUp(true);
            try {
                await axios.post(`${URL}/g/gameStatus`, { winner: winner, gameId: gameId }, { cancelToken: signal.token });
            } catch (error) {
    ...
    useEffect(() => {
        const JoinGame = async () => {
            try {
                ...
                const user = await axios.post(`${URL}/u/verifyToken`, { token: token });
                ...
                if (isMounted.current) {
                    ...
                    game.data.fen.split(' ')[1] === 'w' ? setTurn('white') : setTurn('black');
                    ...
                    socket.on('receiveMove', (data) => {
                        ...
                        if (data.playerColor !== playerColorRef.current) {
                            ...
                            if (isMounted.current) {
                                setFen(chess.fen());
                                setMoves([...movesRef.current, { from: data.move.from, to: data.move.to }]);
                                setInCheck(chess.in_check() && !chess.in_checkmate());
                ...
                if (axios.isCancel(error)) console.error('Error: ', error.message);
    ...
    const handleMove = async (move) => {
        ...
        if (chess.move(move)) {
            ...
            try {
                await axios.post(`${URL}/g/postMove`, { gameId: gameId, move: move, fen: newFen }, { cancelToken: signal.token });
            } catch (error) {
                if (axios.isCancel(error)) {
                    console.error('Error: ', error.message);
                }
    ...
    if (redirectToLogin) {
        return (
            <Redirect
                to={{
                    pathname: '/login',
                    state: { from: props.location.pathname, message: 'Login to play the game' },
                }}
            />
        );
    }
    ...
    if (isLobbyFull) {
        return <Alert variant='warning'>Lobby is already full and game is started. Create new game to play with friends</Alert>;
    }
                            ...
                            {inCheck ? (
                                <tr>
                                    <td colSpan='2' id='in-check-text'>
                                        {turn.charAt(0).toUpperCase() + turn.slice(1)} in Check
                                    </td>
                                </tr>
                            ) : (
                                <></>
